...
namespace TSP.DataManager.TechnicalServices
{
    public class ProjectRequestManager : BaseObject
    {
        ...
        public string DeleteProjectRequest(int ProjectId, int PrjReqId, int WorkFlowCode,int CurrentUserMeId)
        {
            string Result = string.Empty;
            ///Write By AR.Ahmadpour 14-02-30
            try
            {
                CapacityCalculations CapacityCalculations = new CapacityCalculations();
                ...
                ////
                DataTable dt = TSObserverWorkRequestManager2.SelectForObserverWorkRequest(ProjectId, PrjReqId);

                int _MeId;
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    _MeId = Convert.ToInt32(dt.Rows[i]["MeId"]);
                    if (CapacityCalculations.UpdateWorkRequestCapacityData(TSObserverWorkRequestManager, ProjectCapacityDecrementManager, _MeId, CurrentUserMeId, -2, -2, false, TSP.DataManager.TSProjectIngridientType.Nothing, null, false, false) != 0)
                        break;
                }               
            }
            catch (Exception ex)
        ...
        public string DeleteProjectInsertRequest(int ProjectId, int PrjReqId, int WorkFlowCode,int CurrentUserMeId)
        {                     
            string Result = string.Empty;
            ///Write By AR.Ahmadpour 14-02-30
            try
