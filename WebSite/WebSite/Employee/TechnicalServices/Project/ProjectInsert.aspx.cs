...
public partial class Employee_TechnicalServices_Project_ProjectInsert : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            TempMunicipalityId = long.Parse(Request.QueryString["TempId"]);
            SetValuesWithProjectTempMunicipality(TempMunicipalityId);
        }
    }
    #region btn Click
    ...
    /// <summary>
    /// Write By AR.Ahmadpour
    /// </summary>
    private void SetValuesWithProjectTempMunicipality(long TempId)
    {
        try
        {
            TSP.DataManager.TechnicalServices.ProjectManager MunicipalityProj = new TSP.DataManager.TechnicalServices.ProjectManager();
            DataTable Result = MunicipalityProj.GetOneShahrdariProjectForShow(TempId);

            //        شماره ارجاع(کد کامپوتری)    ErjaNumber
            ASPxTextBoxComputerCode.Text = Result.Rows[0][4].ToString();

            //کد نوسازی شهرداری NosaziCode
            ASPxTextBoxReconstructionCode.Text = Result.Rows[0][5].ToString();

            //نوع پروژه ProjectTypeShahrdari
            ASPxComboBoxDiscountPercent.SelectedIndex = (int.Parse(Result.Rows[0][7].ToString())) - 1;

            //منطقه شهرداری Region
            ASPxComboBoxMunicipality.SelectedIndex = (int.Parse(Result.Rows[0][8].ToString())) - 1;




            //آدرس    BuildAddress
            TextBoxAddress.Text = Result.Rows[0][9].ToString();

            //شماره پلاک ثبتی اصلی        MainPelakNumber
            txtRegisteredNo.Text = Result.Rows[0][10].ToString();

            //قطعه Piece
            txtMainSection.Text = Result.Rows[0][11].ToString();

            //ناحیه       District
            txtMainRegion.Text = Result.Rows[0][12].ToString();



            string json = Result.Rows[0][16].ToString();
            DataTable PreBuildingNewList = JsonConvert.DeserializeObject<DataTable>(json);

            //زیر بنای کل     "فیلد 15 از آرایه جدول کنترل نقشه"

            txtProjectFoundation.Text = PreBuildingNewList.Rows[0][4].ToString();

            //تعداد طبقات از روی شالوده   "فیلد 12 از آرایه جدول کنترل نقشه"
            txtStageNum.Text = PreBuildingNewList.Rows[0][1].ToString();


            //نوع سقف "فیلد 18 از آرایه جدول کنترل نقشه"
            ComboBoxRoofType.SelectedIndex = 1; //int.Parse( PreBuildingNewList.Rows[0][18].ToString())-1;

            //نوع اسکلت       "فیلد 19 از آرایه جدول کنترل نقشه"
            ComboBoxStructureSkeleton.SelectedIndex = int.Parse(PreBuildingNewList.Rows[0][8].ToString()) - 1;
            ...
            //نام نماینده مالکین	=> خالی نمایش داده شود

            //نام خانوادگی نماینده مالکین OwnerAgentFullName
            txtOwnerLastName.Text = Result.Rows[0][18].ToString();

            //کد ملی مالک     OwnerAgentNationalCode
            txtSSN.Text = Result.Rows[0][19].ToString();

            //شماره همراه مالک OwnerAgentMobile
            txtOwnerMobileNo.Text = Result.Rows[0][20].ToString();

        }
        catch
        {
            SetLabelWarning("امکان بازیابی اطلاعات ارسالی از شهرداری میسر نمیباشد.");
            return;
        }
    ...
    private void Insert()
    {
        ...
        try
        {
            ...
            {
                TempMunicipalityId = long.Parse(Request.QueryString["TempId"]);
                ProjectManager.UpdateProjectIdInShahrdari(_ProjectId, TempMunicipalityId);
            }
